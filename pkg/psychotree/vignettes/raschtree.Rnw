...
avail <- require("stablelearner", quietly = TRUE)
@

\SweaveOpts{eval = \Sexpr{avail}}
...
The variable \code{spon}, on the other had, was selected in \Sexpr{100 * summary(my_first_raschtree_st)$vstab["spon", 1]}\% of the trees,
also in the orignal tree, on average \Sexpr{summary(my_first_raschtree_st)$vstab["spon", 3]} times, but twice in the original tree. 
